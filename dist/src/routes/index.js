...
router.post("/add", async (req, res) => {
    ...
    try {
        let user = await User_1.User.findOne({ name: new RegExp(`^${name}$`, "i") });
        if (user) {
            user.todos.push({
                todo,
                checked: false
...
router.get("/todos/:name", async (req, res) => {
    ...
    try {
        const user = await User_1.User.findOne({ name: new RegExp(`^${name}$`, "i") });
        if (user) {
...
router.delete("/delete", async (req, res) => {
    const { name } = req.body;
    try {
        const result = await User_1.User.deleteOne({ name: new RegExp(`^${name}$`, "i") });
